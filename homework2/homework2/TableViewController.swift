...
class TableViewController: UITableViewController {
    ...
    @IBAction func delCell(_ sender: UIButton) {
        if !tableView.isEditing {
            sender.setTitle("返回", for: .normal)
        }
        else {
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.imageView?.image = UIImage(named: "image/\(names[indexPath.row]).jpg")
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "infoSegue" {
            ...
            if let row = tableView.indexPathForSelectedRow?.row {
                controller?.imageName = "image/\(names[row]).jpg"
                controller?.itemName = names[row]
            }
        }
    }
}
